...
# These files should be created from your landmark dataset training.
try:
...
while True:
    ...
    # Default prediction if no hand detected.
    ...
    cnn_letter = ""
    landmark_letter = ""
    cnn_confidence = 0.0  # We'll try to get max probability from CNN's fine model if possible.
    ...
    if results.multi_hand_landmarks:
        ...
        # Extract the raw landmark vector (using x, y, z for 21 landmarks => length 63)
        landmark_vector = []
        for lm in hand_landmarks.landmark:
            landmark_vector.extend([lm.x, lm.y, lm.z])
        landmark_vector = np.array(landmark_vector).reshape(1, -1)
        if landmark_model is not None:
            # Scale features using the previously saved scaler.
        ...
        # Simple rule: if CNN's fine confidence is above 0.6, use its prediction; otherwise, use the landmark model.
        if cnn_confidence >= 0.5:
            final_prediction = cnn_letter
        else:
            # If landmark prediction exists, use it.
    ...
    # (Optional) Show the skeleton canvas for debugging in a separate window.
